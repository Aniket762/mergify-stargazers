...
4. Since, the application is merely 70 lines of code, it has not been modularized. With the increasing code complexity and functionality modularization becomes important. For example, having all the routes in `routes` directory and middlewares in `utils` directory and so on. 
5. Moreover, the current application is a monolith since, we only have one single functionality. With increasing functions shifting towards `microservices` is a wise descision. 
